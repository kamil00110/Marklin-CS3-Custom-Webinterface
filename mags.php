...
if ($data) {
    ...
    foreach ($jsonData as $part) {
		...
		if($isInMagList == true and $isS88Contact == false and $states2 == "2"){
			if($state == "0"){
                echo "	    
				<button class='button' style='width=200px;' onclick='sendWs(\"mag\",\"none\",\"none\",\"$id\",\"1\")''>
				...
				echo "	  
                <button class='button' style='width=200px;' onclick='sendWs(\"mag\",\"none\",\"none\",\"$id\",\"128\")''>				
		...
		if($isInMagList == true and $isS88Contact == false and $states2 > "2"){
				$newstate = $state + 1;
                echo "	    
				<button class='button' style='width=200px;' onclick='sendWs(\"mag\",\"none\",\"none\",\"$id\",\"$newstate\")''>
		    	<div style='box-shadow: inset 0px 0px 5px grey; position: relative; padding-left: 7px; padding-right: 7px; margin: 10px; border-radius: 5px; width:50px; height:65px; background-color: #d1c6ae; overflow: hide;'>
		    	<div style='text-align: center; font-size: 11px; padding-top:6px; width: 40px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;'>$name</div>
		    	<img src='$iconFile' style='vertical-align: text-bottom;'; max-height='40px' width='40px'>
				</button>";
